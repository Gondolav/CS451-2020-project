...
for key, value in logs.items():
    logs_pld = list(filter(lambda l: l[0] == 'd' in l, value))
    for d in logs_pld:
        soft_assert(any(l[0] == 'b' and l[1:] == d[2:] for l in logs[int(d[1])]), "Message {} is delivered while not sent ".format(d))
...
for key, value in logs.items():
    ...
    for m in logs_pls:
        soft_assert(['d', m[1], key] in logs[int(m[1])], "Message {} is never delivered".format(m))
